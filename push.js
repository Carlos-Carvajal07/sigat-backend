...
if (process.env.VAPID_PUBLIC_KEY && process.env.VAPID_PRIVATE_KEY) {
  webpush.setVapidDetails(process.env.VAPID_EMAIL||'mailto:admin@institucion.cl',
...
router.post('/subscribe', auth, async (req, res) => {
  ...
  await db.query(
    `INSERT INTO push_subscriptions(usuario_id,endpoint,p256dh,auth)
     VALUES($1,$2,$3,$4) ON CONFLICT(endpoint) DO UPDATE SET usuario_id=$1,p256dh=$3,auth=$4`,
...
router.put('/notificaciones/:id/leer', auth, async (req, res) => {
  await db.query('UPDATE notificaciones SET leida=true WHERE id=$1 AND usuario_id=$2',
