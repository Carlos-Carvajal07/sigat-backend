...
router.post('/login', async (req, res) => {
  try {
    ...
    const u = r.rows[0];
    if (!u || u.estado !== 'Activo') return res.status(401).json({ error: 'Credenciales incorrectas' });
    ...
    res.json({ token, refreshToken: refresh, user: { id:u.id, login:u.login, nombre:u.nombre, rol:u.rol, area:u.area, carrera:u.carrera, correo:u.correo } });
